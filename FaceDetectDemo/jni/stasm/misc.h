...
static inline double PointDist(double x1, // in
		double y1, // in
		double x2, // in
		double y2) // in
		{
...
static inline double PointDist(const Shape& shape1, // in: the first shape
		const Shape& shape2, // in: the second shape
		int ipoint) // in: the point
		{
	return PointDist(shape1(ipoint, IX), shape1(ipoint, IY), shape2(ipoint, IX),
			shape2(ipoint, IY));
}

static inline double PointDist(const Shape& shape, // in
		int ipoint1, // in: the first point
		int ipoint2) // in: the second point
		{
	return PointDist(shape(ipoint1, IX), shape(ipoint1, IY), shape(ipoint2, IX),
			shape(ipoint2, IY));
}
...
struct DetPar // the structure describing a face detection
{
	...
	:
			x(INVALID), y(INVALID), width(INVALID), height(INVALID), lex(
					INVALID), ley(INVALID), rex(INVALID), rey(INVALID), mouthx(
					INVALID), mouthy(INVALID), rot(INVALID), yaw(INVALID), eyaw(
					EYAW(INVALID)) {
	}
	;
...
void splitpath(const char* path, char* drive, char* dir, char* base, char* ext);

void makepath(char* path, const char* drive, const char* dir, const char* base,
		const char* ext);
...
void ShapeMinMax(double& xmin, // out
		double& xmax, // out
		double& ymin, // out
		double& ymax, // out
...
void DesaturateImg(CImage& img); // io: convert to gray (but still an RGB image)
